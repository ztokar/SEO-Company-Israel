...
const Services = () => {
  ...
  const services = [
    ...
    { title: 'Technical SEO', desc: 'Crawlability, Core Web Vitals, and indexing optimization.', link: null }
  ];
        ...
        <div className="grid md:grid-cols-2 gap-16 items-start">
          <div>
            ...
            <div className="space-y-4">
              {services.map((s, i) => (
                s.link ? (
                  <Link key={i} to={s.link} className="flex gap-4 p-5 rounded-2xl border border-gray-100 transition-all cursor-pointer hover:border-primary/40 bg-primary/5">
                    ...
                    <div>
                      <h4 className="font-black text-navy text-sm mb-1">{s.title} <span className="text-[9px] bg-primary text-white px-1.5 py-0.5 rounded ml-1 uppercase font-black">NEW</span></h4>
                      <p className="text-xs text-gray-500 leading-normal font-medium">{s.desc}</p>
                    </div>
                  </Link>
                ) : (
                  <div key={i} className="flex gap-4 p-5 rounded-2xl border border-gray-100 transition-all hover:border-primary/20">
                    <div className="w-10 h-10 bg-primary/10 text-primary flex items-center justify-center rounded-lg flex-shrink-0">
